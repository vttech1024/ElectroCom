    ...
    <section className="volt-auth-wrapper">
      <div className="volt-auth-card">
        ...
        <div className="volt-auth-heading">
          <div className="volt-label">GET STARTED</div>
          <h1 className="volt-auth-title">Create Account</h1>
        </div>

        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
          <div>
            <label className="volt-field-label">FULL NAME</label>
            <input className="volt-input" placeholder="Your name" {...register("name", { required: "Name is required" })} />
            {errors.name && <p className="volt-field-error">{errors.name.message}</p>}
          </div>
          <div>
        ...
        <p className="volt-auth-footer">
          ...
          <Link className="volt-auth-link" to="/login">Sign in →</Link>
        </p>
      </div>
    </section>
  );
